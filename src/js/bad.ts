...
var tournamentMode : boolean = false;
...
let suppressUnload  : boolean = false;
...
let startGame = () => {
    ...
    if (sessionStorage.getItem("tournament") === "1") {
        ...
        if (dataGame === null) { window.location.href = '/tournoi'; return; }

        tournamentMode = true;
        game = new Badminton( JSON.parse(dataGame) as dataSendInfoStart );
        startMatch();
        return;
    ...
    private async newSet ( init : boolean = false ) : Promise<void> {
        ...
        if ((p1 >= this.gameInfos.set && Math.abs(p1 - p2) >= 2) || (p2 >= this.gameInfos.set && Math.abs(p2 - p1) >= 2)) {
            ...
            if (winPlayer.getScore() === this.gameInfos.sets) {
                ...
                if (tournamentMode) { tournoiReportAndReturn( win, this.player1.getScore(), this.player2.getScore() ); return; }
...
/** Joueur d'un tournoi */
interface tournoiPlayer {
    name   : string,
    wins   : number,
    losses : number
}

/** Match d'un tournoi (poule, barrage ou phase finale) */
interface tournoiMatch {
    p1     : number,        // Index du joueur 1
    p2     : number,        // Index du joueur 2
    winner : number | null, // Index du gagnant (null si non joué)
    phase  : string,        // "Poule", "Barrage", "Quart de finale", ...
    s1     : number,        // Jeux gagnés par le joueur 1
    s2     : number         // Jeux gagnés par le joueur 2
}
...
interface tournoiState {
    ...
    started      : boolean,
    roundPlayers : number[],          // Joueurs (indices) du tour de phase finale en cours
    barrageGroup : number[],          // Joueurs à départager
    barrageSpots : number,            // Places à attribuer lors du barrage
    qualified    : number[],          // Qualifiés actés, dans l'ordre des têtes de série
...
let tournoiPhaseLabel = ( size : number ) : string => {
    switch (size) {
...
let tournoiLoad = () : tournoiState | null => {
    let raw = localStorage.getItem("savedTournament");
    return raw === null ? null : JSON.parse(raw) as tournoiState;
}

/** Sauvegarder l'état du tournoi */
let tournoiSave = ( st : tournoiState ) : void => {
    localStorage.setItem("savedTournament", JSON.stringify(st));
}

/** Mélanger un tableau (Fisher-Yates) */
let tournoiShuffle = ( arr : number[] ) : number[] => {
...
let tournoiFinalSizes = ( nbPlayers : number ) : number[] => {
    let sizes : number[] = [];
    for (let s = 2; s <= nbPlayers; s *= 2) sizes.push(s);
    return sizes;
}

/** Générer les matchs de poule (paires aléatoires, sans répéter la paire précédente) */
let tournoiBuildPoule = ( nbPlayers : number, count : number ) : tournoiMatch[] => {

    let matches : tournoiMatch[] = [];
    let prev : string = "";
...
let tournoiBuildFinalRound = ( st : tournoiState ) : void => {
...
let tournoiStartFinal = ( st : tournoiState ) : void => {
...
let tournoiBuildAfterPoule = ( st : tournoiState ) : void => {
...
let tournoiResolveBarrage = ( st : tournoiState ) : void => {

    let bwins : { [k : number] : number } = {};
    st.barrageGroup.forEach(i => bwins[i] = 0);

    st.matches.filter(m => m.phase === "Barrage" && m.winner !== null).forEach(m => {
        if (bwins[m.winner as number] !== undefined) bwins[m.winner as number]++;
    });
...
let tournoiBuildNextFinalRound = ( st : tournoiState ) : boolean => {

    let n         = st.roundPlayers.length / 2;
    let lastRound = st.matches.slice(st.matches.length - n);
    let winners   = lastRound.map(m => m.winner as number);
...
let tournoiEnsureNext = ( st : tournoiState ) : tournoiMatch | null => {
    ...
    while (true) {
        ...
        if      (st.stage === "poule")   tournoiBuildAfterPoule(st);
        else if (st.stage === "barrage") tournoiResolveBarrage(st);
        else if (st.stage === "final") { if (!tournoiBuildNextFinalRound(st)) return null; }
        else                             return null;
...
let tournoiReportAndReturn = ( winSide : number, gamesP1 : number, gamesP2 : number ) : void => {

    let st = tournoiLoad();
    if (st === null) { window.location.href = '/tournoi'; return; }
...
let tournoiGoToMatch = ( st : tournoiState, m : tournoiMatch ) : void => {

    let info : dataSendInfoStart = Object.assign({}, st.gameInfos, {
...
let tournoiShowBoard = () : void => {
    $("#tournoi-config").hide();
    $("#tournoi-board").show();
}

/** Une ligne de match dans le tableau */
let tournoiMatchLine = ( st : tournoiState, m : tournoiMatch, current : boolean ) : string => {

    let n1 = st.players[m.p1].name;
    let n2 = st.players[m.p2].name;

    let res : string;
    if (m.winner === null)                                         res = current ? " <span class=\"vs now\">en cours</span> " : " <span class=\"vs\">vs</span> ";
    else if (typeof m.s1 === "number" && typeof m.s2 === "number") res = " <span class=\"vs\">" + m.s1 + " - " + m.s2 + "</span> ";
    else                                                           res = " <span class=\"vs\">→</span> ";
...
let tournoiRenderBoard = ( st : tournoiState ) : void => {
    ...
    if (poule.length > 0) {
        ...
        st.matches.forEach((m, i) => {
            if (m.phase === "Poule") html += tournoiMatchLine(st, m, i === st.cursor && st.stage === "poule");
        });
        html += `</div>`;
    }
    ...
    if (barrage.length > 0) {
        ...
        st.matches.forEach((m, i) => {
            if (m.phase === "Barrage") html += tournoiMatchLine(st, m, i === st.cursor && st.stage === "barrage");
        });
        html += `</div>`;
    }
    ...
    st.matches.forEach(m => {
        if (m.phase !== "Poule" && m.phase !== "Barrage" && phases.indexOf(m.phase) === -1) phases.push(m.phase);
    });
    if (phases.length > 0) {
        ...
        phases.forEach(ph => {
            ...
            st.matches.forEach((m, i) => {
                if (m.phase === ph) html += tournoiMatchLine(st, m, i === st.cursor && st.stage === "final");
            });
            html += `</div>`;
        });
        html += `</div></div>`;
...
let tournoiAdvanceAndGo = ( st : tournoiState ) : void => {
    ...
    setTimeout(() => tournoiGoToMatch(st, next as tournoiMatch), 4000);

}

/** Bouton Démarrer / Reprendre */
let tournoiStartOrResume = () : void => {
    let st = tournoiLoad();
    if (st !== null) tournoiAdvanceAndGo(st);
}

/** Bouton Nouveau tournoi */
let tournoiNew = () : void => {
    if (!confirm("Démarrer un nouveau tournoi ? Le tournoi en cours sera perdu.")) return;
...
let tournoiAddPlayer = () : void => {
    let n = $(".tournoi-player").length + 1;
    $("#players-list").append(
        `<div class="row-infos player-row">`
...
let tournoiRemovePlayer = ( btn : any ) : void => {
    if ($(".tournoi-player").length <= 2) return;
    $(btn).closest(".player-row").remove();
    tournoiUpdateFinalOptions();
}

/** Mettre à jour les tailles de phase finale proposées selon le nombre de joueurs */
let tournoiUpdateFinalOptions = () : void => {
    ...
    if (prev !== null && sizes.indexOf(Number(prev)) !== -1) $("#finalPhase").val(prev as string);

}

/** Créer le tournoi à partir du formulaire */
let tournoiCreate = () : void => {

    let names : string[] = [];
    $(".tournoi-player").each((i : number, el : any) => {
        let v = ($(el).val() as string).trim();
        names.push(v.length > 0 ? v : "Joueur " + (i + 1));
    });

    if (names.length < 2) { alert("Ajoutez au moins 2 joueurs."); return; }
    ...
    let gameInfos : dataSendInfoStart = {
        player1    : "",
        player2    : "",
    ...
    let players : tournoiPlayer[] = names.map(n => ({ name : n, wins : 0, losses : 0 }));
...
let tournoiInit = () : void => {

    let st = tournoiLoad();

    if (st === null) { tournoiShowConfig(); return; }
...
window.addEventListener('beforeunload', (event) => {
    if (suppressUnload || !matchInProgress) return;
